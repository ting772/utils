 ...
 * @returns 
 */
export function drawLine(ctx: CanvasRenderingContext2D, options?: CanvasCtxOptions, showAngle?: boolean) {
 ...
 * @param options ctx配置对象
 */
export function drawText(ctx: CanvasRenderingContext2D, text: string, pos: Pos, options?: CanvasCtxOptions) {
  ctx.save();
  Object.assign(ctx, options);
 ...
 */
export function drawRect(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number, options?: CanvasCtxOptions, dashed?: number[]) {
  ctx.save();
  Object.assign(ctx, options);
  if (dashed) {
 ...
 * @param options ctx配置对象
 */
export function drawCycle(ctx: CanvasRenderingContext2D, x: number, y: number, r: number, options?: CanvasCtxOptions) {
  ctx.save();
  Object.assign(ctx, options);
  ctx.beginPath();
...
export function setupGrid(options: GridOptions) {
  let { canvas, ctx, width, height, gridSize, lineWidth, gridLineColor } = options;
...
export function setupCoord(options: CoordOptions) {
  ...
  const xScale = (x: number) => x1 + ((x2 - x1) * (x + xMaxAbs)) / (2 * xMaxAbs);

  //取定义域[a,b]中绝对值最大的为y轴尺寸基准，当a,b不以0为中心对称时
  let yMaxAbs = maxAbs(...range);
  const yScale = (y: number) => y1 + ((y2 - y1) * (y + yMaxAbs)) / (2 * yMaxAbs);
...
export function fixedCanvas({ w, h, props }: CreateFixedCanvasOptions) {
  ...
  return {
    ...
    render(draw: (ctx: CanvasRenderingContext2D, canvas: HTMLCanvasElement) => void) {
      draw(ctx!, canvas);
    },
  };
}
